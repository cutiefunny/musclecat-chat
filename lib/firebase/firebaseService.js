...
export const deleteMessage = async (message) => {
  ...
  if (message.imageUrl && (message.type === "photo" || message.type === "emoticon")) {
     // GCS URL에서 경로 추출 (URL 구조가 다양할 수 있음을 고려)
     try {
 ...
 */
export const compressAndUploadImage = async (imageFile, path, compressionOptions, skipCompressionTypes = []) => {
    // ⏹️ GIF 파일인 경우, 압축을 건너뛰고 원본 파일을 바로 업로드합니다.
    if (skipCompressionTypes.includes(imageFile.type)) {
        const fileExtension = imageFile.name.split('.').pop() || '';
        const fullPath = `${path}/${Date.now()}${fileExtension ? '.' + fileExtension : ''}`;
        return uploadImage(imageFile, fullPath);
    }
    ...
    try {
        ...
        try {
            ...
            console.warn('WebP compression failed, using original file type:', error);
            const compressedFile = await imageCompression(imageFile, options);
            const fileExtension = compressedFile.name.split('.').pop() || 'jpeg';
            return uploadImage(compressedFile, `${path}/${Date.now()}.${fileExtension}`);
        }
    }
};
...
export const addEmoticon = async (imageFile, order) => {
    // ⏹️ 이모티콘에 맞는 압축 옵션과 GIF 제외 설정을 전달합니다.
    const emoticonCompressionOptions = {
        maxSizeKB: 100,
        maxWidthOrHeight: 200,
    };
    const url = await compressAndUploadImage(imageFile, 'emoticons', emoticonCompressionOptions, ['image/gif']);
