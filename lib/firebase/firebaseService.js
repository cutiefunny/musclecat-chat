...
/**
 * 새로운 메시지를 전송합니다.
 * @param {object} messageData - { text, imageUrl, storagePath, type, sender, uid, authUid, replyTo }
  ...
  if (message.imageUrl && (message.type === "photo" || message.type === "emoticon")) {
     try {
        ...
        if (message.storagePath) {
            // New, robust way
            imageRef = ref(storage, message.storagePath);
        } else {
            // Old, fragile way (fallback)
            const imagePath = new URL(message.imageUrl).pathname.split('/o/')[1].split('?')[0];
            const decodedPath = decodeURIComponent(imagePath);
            imageRef = ref(storage, decodedPath);
        }
 ...
 * @returns {Promise<{downloadURL: string, storagePath: string}>} - 업로드된 이미지 URL과 스토리지 경로
 */
export const compressAndUploadImage = async (imageFile, path, customOptions = {}) => {
  ...
  for (const attempt of compressionAttempts) {
    try {
      const filePath = `${path}/${Date.now()}.${attempt.extension}`;
      const compressedFile = await imageCompression(imageFile, { ...options, fileType: attempt.type });
      return await uploadImage(compressedFile, filePath);
    } catch (error) {
  ...
  const originalPath = `${path}/${Date.now()}`;
  return await uploadImage(imageFile, originalPath);
};
 ...
 * @returns {Promise<{downloadURL: string, storagePath: string}>} - 업로드된 파일 URL과 스토리지 경로
 */
export const uploadImage = async (file, fullPath) => {
  const storageRef = ref(storage, fullPath);
  await uploadBytes(storageRef, file);
  const downloadURL = await getDownloadURL(storageRef);
  return { downloadURL, storagePath: fullPath };
};
...
export const addEmoticon = async (imageFile, category, order) => {
    ...
    const { downloadURL, storagePath } = await compressAndUploadImage(imageFile, `emoticons/${category}`, emoticonOptions);
...
export const deleteEmoticon = async (emoticon) => {
    ...
    if (emoticon.storagePath) {
        const imageRef = ref(storage, emoticon.storagePath);
        await deleteObject(imageRef);
    } else if (emoticon.url) { // Fallback for old data
...
export const requestNotificationPermission = async () => {
    try {
        ...
        if (permission === "granted") {
            // Service Worker 명시적 등록
            let serviceWorkerRegistration;
            
            if ('serviceWorker' in navigator) {
                try {
                    // route.js가 제공하는 SW 파일 경로 등록
...
/**
 * 공지사항을 실시간으로 구독합니다.
 * @param {function} onNoticeChange - 공지사항 객체를 인자로 받는 콜백
 * @returns {function} - Unsubscribe 함수
 */
export const subscribeToNotice = (onNoticeChange) => {
  const noticeRef = doc(db, "settings", "notice");
  return onSnapshot(noticeRef, (docSnap) => {
    if (docSnap.exists()) {
      onNoticeChange(docSnap.data());
    } else {
      const defaultNotice = { text: "", isVisible: false };
      setDoc(noticeRef, defaultNotice);
      onNoticeChange(defaultNotice);
    }
  });
};
...
/**
 * 공지사항을 업데이트합니다.
 * @param {object} newNotice - 업데이트할 공지사항 객체 (예: { text: "공지입니다", isVisible: true })
 */
export const updateNotice = async (newNotice) => {
  const noticeRef = doc(db, "settings", "notice");
  await setDoc(noticeRef, newNotice, { merge: true });
};


/**
 * UI 설정(메시지 모달 활성화 여부 등)을 실시간으로 구독합니다.
 * @param {function} onSettingsChange - 설정 객체를 인자로 받는 콜백
 * @returns {function} - Unsubscribe 함수
 */
export const subscribeToUiSettings = (onSettingsChange) => {
  const settingsRef = doc(db, "settings", "ui");
  return onSnapshot(settingsRef, (docSnap) => {
    if (docSnap.exists()) {
      onSettingsChange(docSnap.data());
    } else {
      // 문서가 없으면 기본값 생성
      const defaultSettings = { isMessageModalActive: true };
...
/**
 * UI 설정을 업데이트합니다.
 * @param {object} newSettings - 업데이트할 설정 객체 (예: { isMessageModalActive: false })
 */
export const updateUiSettings = async (newSettings) => {
  const settingsRef = doc(db, "settings", "ui");
