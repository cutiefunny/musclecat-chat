...
export const sendMessage = async (messageData) => {
  await addDoc(collection(db, "messages"), {
    ...
    reactions: [],
    readBy: [messageData.authUid], // 💡 보낸 사람을 바로 읽음 처리
  });
};
...
export const updateUserProfile = async (uid, data) => {
  ...
  // 💡 문서를 안전하게 생성하거나 업데이트하기 위해 setDoc과 merge:true를 사용합니다.
...
export const getUserProfile = async (user) => {
    ...
    if (!userSnap.exists()) {
      const newUser = {
        // 💡 익명 로그인 시 displayName이 없으므로 null로 초기화합니다.
